...
export async function GET(request: NextRequest) {
  try {
    ...
    const stats = {
      ...
      whatsapp_active: unifiedUsers.filter(
        (u) => u.whatsapp_consent && !u.whatsapp_revoked_at,
      ).length,
      marketing_active: unifiedUsers.filter(
        (u) => u.marketing_consent && !u.marketing_revoked_at,
      ).length,
    };
