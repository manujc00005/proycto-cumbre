...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!currentMember) {
      return NextResponse.json(
        { error: 'Solo se pueden eliminar miembros registrados. Los participantes de eventos no se pueden eliminar.' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: 'Usuario eliminado correctamente. Los datos se conservan para cumplimiento legal.',
      member: {
        id: member.id,
        member_number: currentMember.member_number,
        deleted_at: member.deleted_at
