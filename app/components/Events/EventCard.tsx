...
const EventCard: React.FC<EventCardProps> = ({ event, dropdownId }) => {
  ...
  return (
    <div
      className={`relative pl-20 group ${config.opacity} transition-opacity`}
    >
      <div
        className={`absolute left-6 top-6 w-5 h-5 rounded-full ${config.dotColor} border-4 border-zinc-900 group-hover:scale-125 transition-transform shadow-lg ${config.dotShadow}`}
      ></div>

      <div
        className={`${cardBgClass} p-6 rounded-xl border ${config.borderColor} transition-all shadow-lg ${config.cardShadow}`}
      >
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2 flex-wrap">
              <span
                className={`px-3 py-1 ${config.badge.bgColor} ${config.badge.textColor} text-xs font-bold rounded-full`}
              >
                {config.badge.text}
              </span>
              <span className={`${config.dateColor} font-semibold text-sm`}>
                {event.date}
              </span>
            </div>
            <h4
              className={`${isCompleted ? "text-lg" : "text-xl"} font-bold ${config.titleColor} mb-2`}
            >
              {event.title}
            </h4>
        ...
        {showDetails && (event.tags || status === "upcoming") && (
          <div
            className={`flex ${status === "upcoming" ? "justify-between" : "gap-2"} items-center flex-wrap gap-2 mt-4`}
          >
            {event.tags && (
            ...
            {status === "upcoming" && dropdownId && (
              <AddToCalendar event={calendarData} dropdownId={dropdownId} />
            )}
          </div>
        )}
      </div>
