...
export function generateCalendarData(event: Event) {
  ...
  const hasNocheTag = event.tags?.some(
    (tag) => tag.label.toLowerCase() === "noche",
  );
  const hasTardeTag = event.tags?.some(
    (tag) => tag.label.toLowerCase() === "tarde",
  );
  const hasMañanaTag = event.tags?.some(
    (tag) => tag.label.toLowerCase() === "mañana",
  );
  ...
  const description =
    event.description || `Evento del club de montaña: ${event.title}`;
