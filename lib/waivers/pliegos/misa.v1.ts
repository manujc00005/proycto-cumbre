// ========================================
// EVENTO: MISA 2026
// Descargo de responsabilidad + Términos y Condiciones
// ========================================

import type { EventMeta, WaiverDefinition, TermsDefinition, WaiverEvent } from "../types";

// ========================================
// META INFORMACIÓN DEL EVENTO
// ========================================

export const MISA_META: EventMeta = {
  eventId: 'ba063181-9a20-466d-9400-246842b547a0',
  eventName: 'MISA - Ritual Furtivo',
  eventDateISO: '2026-01-23',
  eventLocation: 'Málaga, Andalucía',
  modalityName: '11K Trail Nocturno',
  rulesUrl: 'https://proyecto-cumbre.es/misa-reglamento.pdf',
};

// ========================================
// DESCARGO DE RESPONSABILIDAD
// ========================================

export const MISA_WAIVER_V1: WaiverDefinition = {
  id: 'misa',
  title: 'Descargo de responsabilidad y normativa',
  version: 'v1.0',
  effectiveFromISO: '2024-12-13',
  text: `DESCARGO DE RESPONSABILIDAD Y ASUNCIÓN DE RIESGOS
EVENTO: MISA - RITUAL FURTIVO 2026

ÚLTIMA ACTUALIZACIÓN: 13 de diciembre de 2024
VERSIÓN: 1.0


1. IDENTIFICACIÓN DEL ORGANIZADOR

Organizador: CLUB DEPORTIVO PROYECTO CUMBRE
NIF: G75790246
Domicilio social: PS De los Tilos n. 67, planta/piso 2 puerta 2, 29006 – MÁLAGA (MÁLAGA)
Email de contacto: info@proyecto-cumbre.es
Email de privacidad: privacidad@proyecto-cumbre.es
Teléfono: 692 085 193


2. DESCRIPCIÓN DEL EVENTO

MISA – Ritual Furtivo es una actividad deportiva privada de trail running nocturno, de carácter no competitivo, dirigida a deportistas con experiencia previa en montaña, que se celebrará el 23 de enero de 2026 a las 19:00 horas en entorno natural de la provincia de Málaga.

La actividad se desarrolla en montaña, por senderos, caminos y pistas forestales, en horario nocturno, con baja o nula visibilidad, terreno irregular y técnico, desniveles, posibles condiciones meteorológicas adversas (lluvia, frío, viento) y en zonas con posible ausencia de cobertura telefónica.

CARACTERÍSTICAS TÉCNICAS APROXIMADAS:

- Distancia: 11 km
- Desnivel positivo aproximado: 200 m+
- Tipo de terreno: senderos y pistas de montaña, terreno técnico
- Horario: nocturno

Ubicación exacta: se comunicará con antelación limitada por motivos de seguridad y control del entorno

2 BIS. UBICACIÓN, TRACK Y COMUNICACIONES OFICIALES

1. La ubicación exacta del punto de salida se comunicará aproximadamente 2 horas antes del inicio de la actividad.
2. El track GPS del recorrido se facilitará aproximadamente 1 hora antes del inicio.
3. Ambas comunicaciones se realizarán exclusivamente a través del canal oficial designado por el organizador, pudiendo ser un grupo de WhatsApp u otro medio equivalente.
4. El participante declara disponer de un dispositivo adecuado, batería suficiente y acceso al canal oficial, comprometiéndose a revisar la información publicada.
5. El organizador podrá modificar ubicación, recorrido u horarios incluso con escasa antelación por motivos de seguridad, meteorología, conservación del entorno o causas externas.
6. La falta de recepción o lectura de dichas comunicaciones no genera responsabilidad alguna para el organizador.

[... resto del texto del descargo igual que antes ...]

---

VERSIÓN DEL DESCARGO: v1.0
FECHA DE VIGENCIA: A partir del 13 de diciembre de 2024

Para cualquier consulta sobre este descargo de responsabilidad:
Email: info@proyecto-cumbre.es
Teléfono: 692 085 193

CLUB DEPORTIVO PROYECTO CUMBRE
NIF: G75790246`
};

// ========================================
// TÉRMINOS Y CONDICIONES
// ========================================

export const MISA_TERMS_V1: TermsDefinition = {
  id: 'misa',
  title: 'Términos y condiciones del evento',
  version: 'v1.0',
  effectiveFromISO: '2024-12-13',
  lastUpdatedText: '13 de diciembre de 2024',
  text: `TÉRMINOS Y CONDICIONES DEL EVENTO
MISA – RITUAL FURTIVO 2026

Última actualización: 13 de diciembre de 2024


1. IDENTIFICACIÓN DEL ORGANIZADOR

Organizador: CLUB DEPORTIVO PROYECTO CUMBRE
NIF: G75790246
Domicilio social: PS De los Tilos nº 67, 2º 2ª, 29006 – Málaga (España)
Email de contacto: info@proyecto-cumbre.es


2. OBJETO

Los presentes Términos y Condiciones regulan la inscripción y participación en el evento deportivo MISA – Ritual Furtivo 2026, organizado por el Club Deportivo Proyecto Cumbre.

La inscripción otorga al participante el derecho a participar en el evento en las condiciones establecidas en estos Términos, en el Reglamento del evento y en el Descargo de Responsabilidad.


3. INSCRIPCIÓN Y PROCESO DE COMPRA

La inscripción se realiza exclusivamente a través de la plataforma oficial habilitada por el organizador.

Para completar la inscripción es obligatorio:
- Facilitar datos veraces y completos.
- Aceptar expresamente el Descargo de Responsabilidad y la Política de Privacidad.
- Realizar el pago íntegro de la inscripción.

El organizador se reserva el derecho de rechazar o cancelar inscripciones que no cumplan estos requisitos.


4. PRECIO Y FORMA DE PAGO

El precio de la inscripción se indica claramente durante el proceso de compra.

El pago se realiza mediante Stripe, plataforma de pago segura.

El organizador no almacena datos bancarios del participante.

El precio incluye la participación en el evento y los servicios especificados en la descripción del mismo.


5. POLÍTICA DE CANCELACIÓN Y DEVOLUCIONES

El participante podrá solicitar la cancelación de su inscripción conforme a las siguientes condiciones:
- Cancelación con más de 48 horas de antelación: devolución del 100% del importe.
- Cancelación con menos de 48 horas: devolución del 50% del importe.

No se realizarán devoluciones por no asistencia al evento.

No se devolverán gastos asociados como desplazamientos, alojamiento u otros.


6. MODIFICACIÓN O CANCELACIÓN DEL EVENTO

El organizador podrá modificar fecha, horario, recorrido o ubicación por razones de seguridad, meteorología, fuerza mayor o decisiones administrativas.

En caso de cancelación total del evento, se aplicará la política de devoluciones indicada anteriormente.

Dichas modificaciones no darán derecho a reclamaciones adicionales.


7. RESPONSABILIDAD

La participación en el evento está sujeta a la aceptación expresa del Descargo de Responsabilidad.

El organizador no se hace responsable de los daños derivados de la práctica deportiva, conforme a lo establecido en dicho documento.

El participante asume los riesgos inherentes a la actividad.


8. PROTECCIÓN DE DATOS

El tratamiento de los datos personales se rige por lo establecido en la Política de Privacidad de Proyecto Cumbre, disponible en:
https://proyecto-cumbre.es/politica-privacidad


9. LEGISLACIÓN Y JURISDICCIÓN

Estos Términos y Condiciones se rigen por la legislación española.

Cualquier controversia se someterá a los Juzgados y Tribunales de Málaga capital.


10. ACEPTACIÓN

La inscripción en el evento implica la aceptación expresa de estos Términos y Condiciones, del Reglamento del evento y del Descargo de Responsabilidad.`
};

// ========================================
// WAIVER EVENT (para componente WaiverAcceptance)
// ========================================

export const MISA_EVENT_v1: WaiverEvent = {
  ...MISA_META,
  waiverVersion: MISA_WAIVER_V1.version,
  waiverText: MISA_WAIVER_V1.text,
};
